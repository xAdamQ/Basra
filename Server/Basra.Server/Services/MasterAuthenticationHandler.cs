...
namespace Basra.Server.Services
{
        ...
        MasterAuthenticationHandler : AuthenticationHandler<MasterAuthenticationSchemeOptions>
    {
        ...
        private ILogger<MasterAuthenticationHandler> _logger;
        ...
        */
        ...
        protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
        {
            ...
            try
            {
                ...
                if (String.IsNullOrEmpty(demo)) //not demo
                {
                    if (!string.IsNullOrEmpty(huaweiAuthCode))
                    {
                        ...
                        try
                        {
                            ...
                            user = await _securityManager.SignInAsync(userData.openId,
                    ...
                    {
                        if (!_securityManager.ValidateToken(figToken, out var playerId))
                            return AuthenticateResult.Fail("fig token validation failed");

                        user = await _securityManager.SignInAsync(playerId,
                    ...
                    else if (!string.IsNullOrEmpty(fbAccToken))
                    {
                        try
                        {
                            var isValid = await _securityManager.ValidateFbAccToken(fbAccToken);

                            if (!isValid)
                                return AuthenticateResult.Fail(
                                    "the given fb acc token is not valid");

                            var userData = await _securityManager.GetFbProfile(fbAccToken);

                            user = await _securityManager.SignInAsync(userData.id,
                ...
                {
                    user = await _securityManager.SignInAsync(figToken,
                        (int) ExternalIdType.Demo, name, pictureUrl);
                }

                if (_sessionRepo.IsUserActive(user.Id) &&
                    _sessionRepo.GetActiveUser(user.Id).IsDisconnected == false)
                    return AuthenticateResult.Fail(
