...
namespace Basra.Server
{
    public class RoomUser : IRoomUser
    {
        ...
        public static Dictionary<string, RoomUser> All = new Dictionary<string, RoomUser>();

        public string ConnectionId { get; set; }
        public string UserId { get; set; }
        //context object array for connected users
        ...
        /// when active is made
        /// </summary>
        public async Task StartRoom(ActiveRoom active, int id, string[] playerNames)
        {
            IdInRoom = id;
            ActiveRoom = active;

            await Program.HubContext.Clients.Client(ConnectionId).SendAsync("StartRoom", IdInRoom, playerNames);
        }
        ...
        public async Task InitialDistribute()
        {
            ...
            await Program.HubContext.Clients.Client(ConnectionId).SendAsync("InitialDistribute", Cards.ToArray(), ActiveRoom.GroundCards.ToArray());
        }
        //rev rpc
        ...
        public async Task Play(int cardIndexInHand)
        {
            ...
            var eaten = RoomLogic.Eat(Cards[cardIndexInHand], ActiveRoom.GroundCards, out bool basra, out bool bigBasra);
            ...
            await Program.HubContext.Clients.GroupExcept("room" + ActiveRoom.Id, ConnectionId).SendAsync("CurrentOppoThrow", Cards[cardIndexInHand]);
            //what do you mean by await?, waiting for deliver or timeout?

            if (Cards.Count == 0 && IdInRoom == ActiveRoom.Users.Length - 1)
            {
